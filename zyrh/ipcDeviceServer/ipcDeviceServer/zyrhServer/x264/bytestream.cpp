/*****************************************************************************
 * bytestream.c: flv muxer utilities
 *****************************************************************************
...
int move( buffer *c,   uint32_t  nsize )
{
	memcpy( c->data, c->data + nsize, c->d_cur - nsize );
	c->d_cur = c->d_cur - nsize;
	return 0;
}
...
{
	*(c->data + start + 0) = length >> 8;
	*(c->data + start + 1) = length >> 0;
}
void put_amf_string( buffer *c, const char *str )
{
	uint16_t len = strlen( (char*)str );
	put_be16( c, len );
	append_data( c, (uint8_t*)str, len );
}
...
{
	uint64_t vla32_1 = get_byte32(val);
	val += 4;
	uint64_t vla32_2 = get_byte32(val);
	uint64_t retVal = vla32_1<<32|vla32_2;
	return retVal;
}
...
uint16_t get_amf_string( const char* c,char* dest)
{
	...
	if (c != NULL)
	{	
		strlen = get_byte16( (char*)c );
		memcpy(dest,c + 2,strlen);
		*(dest + strlen ) = 0;
	}
	return strlen;
}
void freeNal(x264_nal_t* pNal)
{
	if (pNal)
	{
		if (pNal->p_payload)
		{
			delete[] pNal->p_payload;
			pNal->p_payload = NULL;
		}
		pNal = NULL;
		delete[] pNal;
...
void MallocNal(x264_nal_t** pNal)
{
	if (pNal)
	{
		freeNal(*pNal);
