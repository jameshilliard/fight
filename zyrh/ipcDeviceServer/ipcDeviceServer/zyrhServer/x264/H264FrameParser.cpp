		...
		bool& isKeyframe)
{
    ...
    while (start < end) 
    {
        ...
        {
			spsBuf->i_payload = size + 4;
			spsBuf->i_type = NAL_SPS;
			spsBuf->p_payload = new uint8_t[size + 4];
			memcpy(spsBuf->p_payload, (char*)h264startcodebuf, 4);
			memcpy(spsBuf->p_payload + 4 , start + 4  , size);
        }
        else if (nal_type == NAL_PPS && ppsBuf)   /* PPS */
        {
			ppsBuf->i_payload = size + 4;
			ppsBuf->i_type = NAL_PPS;
			ppsBuf->p_payload = new uint8_t[size  + 4 ];
			memcpy(ppsBuf->p_payload, (char*)h264startcodebuf, 4);
			memcpy(ppsBuf->p_payload + 4 , start + 4 , size);
        }
		else if (nal_type == NAL_SEI && pSeiBuf)   /* PPS */
		{
			pSeiBuf->i_payload = size + 4;
			pSeiBuf->i_type = NAL_SEI;
			pSeiBuf->p_payload = new uint8_t[size + 4];
			memcpy(pSeiBuf->p_payload, (char*)h264startcodebuf, 4);
			memcpy(pSeiBuf->p_payload + 4, start + 4, size);
		}
        else if (nal_type == NAL_SLICE_IDR)
        {
            isKeyframe = true;
        }
        start += size + 4;
...
void ParseHFlvToNal( char* nalsbuf, int size)
{
	...
	while (start < end) 
	{
		unsigned int size = BytesToUI32(start);
		unsigned char nal_type = start[4] & 0x1f;
		memcpy(start,h264startcodebuf,4);
		start += size + 4;
	}
}
